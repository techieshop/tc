...
$query = "select his_reserve_head.customer_id,customer_no,customer_name from his_reserve_head,ntt_customer where (dn_date >= '$date_fr' and dn_date < '$date_to') and (ntt_customer.customer_id=his_reserve_head.customer_id) group by his_reserve_head.customer_id order by line_no,his_reserve_head.customer_id";
$result = mysql_query($query,$db);
while ($myrow = mysql_fetch_array($result)) {
    printf("<tr>");
    printf("<td align=center>%s</td>",$myrow["customer_no"]);
    printf("<td align=left><a href=javascript:openPrintWindow('dn_sta.php?custno=%s&date_fr=$date_fr&date_to=$date_to','dn_sta',790,450);>%s</a></td>",$myrow["customer_id"],$myrow["customer_name"]);
    printf("</tr>");
}
